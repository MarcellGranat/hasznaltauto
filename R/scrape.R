...
available_cars <- available_cars %>% 
  mutate(page = map(url, SleepyRead)) %>% 
  filter(map_lgl(page, ~ !is.na(.))) %>% 
  mutate(
    url_to_car = map2(page, ".cim-kontener a", GetURL),
    price = map2(page, ".vetelar", GetHTMLText)
  )

available_cars <- available_cars %>% 
  unnest(url_to_car, price) %>% 
  select(url_to_car, price) %>% 
  na.omit()
